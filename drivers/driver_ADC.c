...
#include "math.h"
	...
	static int Iu_adc = 2048, Iv_adc = 2048, Iw_adc = 2048, Idc_adc = 2048, Vdc_adc = 0;	//Direct ADC values, needed for checking faults
	static float Iu_off = 2048, Iv_off = 2048, Iw_off = 2048, Idc_off = 2048;				//Offsets in currents, to be caliberated using 'adc_calib()'
    ...
    static int throttle_error = 0, sensor_error = 0;
    static int poles = 48;
...
void fast_measurement_VI(float *Vdc, float *Idc, float *Iu, float *Iv, float *Iw)
{
    ...
    *Iv = - *Iu - *Iw;
    ...
    tempfv = *Iv * (4096.0/ISCALE);
    Iv_adc =  (int)(tempfv + Iv_off);     
     
    *Idc = (float)Idc_adc;
    *Idc = (*Idc - Idc_off)*(ISCALE / 4096.0);    
    
    fIdc_adc = fIdc_adc + 0.01*((float)Idc_adc - fIdc_adc);
...
void fast_measurement_RP(float *cosMR, float *sinMR, int MRdirection)
{
    ...
    if(MRcosP < 400 || MRcosP > 3696 || MRcosN < 400 || MRcosN > 3696 || MRsinP < 400 || MRsinP > 3696 || MRsinN < 400 || MRsinN > 3696) sensor_error = 1;
    else sensor_error = 0;
    
    //DAC1CONbits.DACDAT = MRcosP;
...
void slow_measurement_T(float *Tesc_adr, float *Imosfet_adr, float *Tm_adr)
{
    ...
    *Imosfet_adr = NSP * sqrt(fabs(2*1000*(TRDS - fTesc)/RDSON/RTJS));
    ...
    DAC1CONbits.DACDAT = (int)(fTm * 4096/330.0);
    DAC2CONbits.DACDAT = (int)(fTm * 4096/330.0);
...
void slow_measurement_input(float sdt, float *adr_wSensor, float * a_input_adr, float *b_input_adr)
{
    ...
    fwMech = fwMech + 0.1*(wMech - fwMech);
    ...
    *adr_wSensor = 5.0*fwMech;
    ...
    if(fThrottle < 0.5 || fThrottle > 4.5) throttle_error = 1;
    else throttle_error = 0;
...
int fast_fault_check(void)
{
    ...
    if(Iv_adc > Ifltp) {fault_id = 2;}
    if(Iw_adc > Ifltp) {fault_id = 2;}
    if(Iu_adc < Ifltn) {fault_id = 2;}
    if(Iv_adc < Ifltn) {fault_id = 2;}
...
void clearable_fault_check(int * fault_id_adr)
{
    if(*fault_id_adr == 0 || *fault_id_adr == 9 || *fault_id_adr == 7)
    {
        if(throttle_error != 0) *fault_id_adr = 9; 
        if(throttle_error == 0 && *fault_id_adr == 9) *fault_id_adr = 0; 
        
        if(sensor_error != 0) *fault_id_adr = 7; 
        if(sensor_error == 0 && *fault_id_adr == 7) *fault_id_adr = 0; 
    }
